...
export const audiencePages: ContentPage[] = [
  {
    ...
    sections: [
      ...
      { heading: "Where this stops", paragraphs: ["AnalyseSpider can fetch one public HTTP or HTTPS URL through a tightly bounded, SSRF-protected gateway. It does not crawl a site, enter authenticated areas, render JavaScript, query Search Console, or decide canonical selection. Its separate browser-local verifier can compare one logged IP and user-agent claim with dated operator-published ranges; that is request-origin evidence, not proof of indexing, training, or citation."], callout: "Client logs can contain personal or confidential data. Minimise the sample before pasting it into any tool, including a local one, and never submit a private or signed URL to the live checker." },
