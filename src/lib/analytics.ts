...
const eventSchemas = {
  navigation_click: {
    ...
    target: ["home", "tools", "crawlers", "learn", "about", "legal", "contextter", "ai_fanout", "github", "other_internal", "other_external"],
  },
  tool_run_started: {
    tool: ["crawler_check", "crawler_comparison", "http_response", "redirect_chain", "robots_rule", "server_log", "ip_classifier", "bot_verification"],
  },
  tool_run_succeeded: {
    tool: ["crawler_check", "crawler_comparison", "http_response", "redirect_chain", "robots_rule", "server_log", "ip_classifier", "bot_verification"],
    outcome: ["yes", "no", "mixed", "unknown", "complete", "verified", "not_verified", "unverifiable"],
  },
  tool_run_failed: {
    tool: ["crawler_check", "crawler_comparison", "http_response", "redirect_chain", "robots_rule", "server_log", "ip_classifier", "bot_verification"],
    reason: ["validation", "rate_limit", "busy", "timeout", "origin", "captcha", "service", "unknown"],
  },
  crawler_profile_selected: {
    profile: ["googlebot", "oai_searchbot", "gptbot", "chatgpt_user", "claude_searchbot", "claudebot", "claude_user", "perplexitybot"],
  },
  report_action: {
    action: ["copy", "download", "compare_previous", "open_redirects", "open_http"],
    outcome: ["success", "failure", "unavailable"],
  },
  tool_preset_used: {
    tool: ["http_response", "redirect_chain", "robots_rule", "server_log", "ip_classifier", "bot_verification"],
  },
...
declare global {
  interface Window {
    umami?: { track: (name: string, data?: Record<string, EventValue>) => void };
  }
}

export function sanitizeEvent(name: EventName, raw: Record<string, unknown>): { name: EventName; data: Record<string, EventValue> } {
...
export function trackEvent(name: EventName, raw: Record<string, unknown>): boolean {
...
function clickLocation(anchor: HTMLAnchorElement): "header" | "main" | "footer" {
...
function clickTarget(anchor: HTMLAnchorElement): string {
  ...
  if (url.hostname === "contextter.com" || url.hostname === "www.contextter.com") return "contextter";
  if (url.hostname === "ai-fanout.com" || url.hostname === "www.ai-fanout.com") return "ai_fanout";
  ...
  if (path.startsWith("/guides") || path.startsWith("/blog") || path.startsWith("/reference") || path.startsWith("/wissen")) return "learn";
  if (path.startsWith("/about") || path.startsWith("/ueber")) return "about";
  if (path.startsWith("/privacy") || path.startsWith("/impressum") || path.startsWith("/contact") || path.startsWith("/methodology")) return "legal";
  return "other_internal";
}

const genericToolActions: Readonly<Record<string, { tool: string; result: string }>> = {
...
function observeToolResult(tool: string, selector: string): void {
  ...
  window.setTimeout(() => { sendIfVisible(); observer.disconnect(); }, 15_000);
  sendIfVisible();
}
...
export function mountAnalytics(): void {
  document.addEventListener("click", (event) => {
    ...
    if (target.closest("[data-ip-preset], [data-robots-preset], [data-log-preset], [data-response-preset], [data-redirect-preset]")) {
      const tool = target.closest("[data-ip-preset]") ? "ip_classifier"
        : target.closest("[data-robots-preset]") ? "robots_rule"
          : target.closest("[data-log-preset]") ? "server_log"
            : target.closest("[data-response-preset]") ? "http_response"
              : "redirect_chain";
